...
**Runtime:** N/A  
**Memory:** N/A  
**Submitted:** 2026-09-09T15:13:07.617Z  
...
void solve() {
    ...
    vector<int> min_pos(k + 1), max_pos(k + 1);
    int cur_min = n + 1, cur_max = 0;
    for (int i = 1; i <= k; ++i) {
        cur_min = min(cur_min, pos[i]);
        cur_max = max(cur_max, pos[i]);
        min_pos[i] = cur_min;
        max_pos[i] = cur_max;
    }
    ...
    for (int i = 1; i <= k; ++i) {
        A[i] = max(1, max_pos[i] - 2 * i + 2);
        B[i] = min(min_pos[i], n - 2 * i + 2);
        if (A[i] > B[i]) {
            cout << -1 << "\n";
            return;
        }
